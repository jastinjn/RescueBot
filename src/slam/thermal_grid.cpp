...
ThermalGrid::ThermalGrid(float widthInMeters,
...
void ThermalGrid::setOrigin(float x, float y){
...
bool ThermalGrid::isCellInGrid(int x, int y) const
{ 
...
thermal_grid_t ThermalGrid::toLCM(void) const
{
    thermal_grid_t grid;
...
void ThermalGrid::fromLCM(const thermal_grid_t& gridMessage)
{
...
bool ThermalGrid::saveToFile(const std::string& filename) const
{
    std::ofstream out(filename);
    if(!out.is_open())
    {
        std::cerr << "ERROR: ThermalGrid::saveToFile: Failed to save to " << filename << '\n';
        return false;
    }
...
bool ThermalGrid::loadFromFile(const std::string& filename)
{
    std::ifstream in(filename);
    if(!in.is_open())
    {
        std::cerr << "ERROR: ThermalGrid::loadFromFile: Failed to load from " << filename << '\n';
        return false;
    }
